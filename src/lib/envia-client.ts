...
export class EnviaClient {
   ...
   *
   * NOT retried on transient errors. Label generation has a real side
   * effect — Envia creates a label and charges credit. If we get a 502
   * / timeout AFTER Envia already created the label server-side (common
   * when the API is fronted by Cloudflare), retrying creates a duplicate
   * label.
   *
   * Both the real-order Envia workflow (envia-create-fulfillment) and
   * the simplified influencer-sample route loop through the carrier
   * pool, so a single transient failure falls back to the next carrier
   * instead of retrying with the same one.
   */
